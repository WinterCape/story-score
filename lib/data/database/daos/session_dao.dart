...
class SessionDao extends DatabaseAccessor<AppDatabase>
  ...
  Stream<GameSession?> watchSession(String id) {
    return (select(gameSessions)..where((s) => s.id.equals(id)))
        .watchSingleOrNull();
  }
  ...
  Stream<List<Player>> watchPlayersForSession(String sessionId) {
    return (select(players)
          ...
          ..orderBy([
            (p) => OrderingTerm.asc(p.seatOrder),
          ]))
        .watch();
  }
  ...
  Future<void> updateSession(
      String id, GameSessionsCompanion companion) async {
    await (update(gameSessions)..where((s) => s.id.equals(id)))
        .write(companion);
  }
  ...
  Future<void> updatePlayerScore(String playerId, int newScore) async {
    await (update(players)..where((p) => p.id.equals(playerId)))
        .write(PlayersCompanion(currentScore: Value(newScore)));
  }
  ...
  Future<void> recomputePlayerScores(String sessionId) {
    return transaction(() async {
      final sessionPlayers = await (select(players)
            ..where((p) => p.sessionId.equals(sessionId)))
          .get();
      ...
      for (final player in sessionPlayers) {
        ...
        await (update(players)..where((p) => p.id.equals(player.id)))
            .write(PlayersCompanion(currentScore: Value(total)));
      }
    });
  }
    ...
    return transaction(() async {
      ...
      if (roundIds.isNotEmpty) {
        ...
        await (delete(scoreChanges)
              ..where((sc) => sc.roundId.isIn(roundIds)))
            .go();
      }
  ...
  Future<void> advanceStoryteller(String sessionId) {
    return transaction(() async {
      final session = await (select(gameSessions)
            ..where((s) => s.id.equals(sessionId)))
          .getSingle();

      final playerCount = await (select(players)
            ..where((p) => p.sessionId.equals(sessionId)))
          .get()
